const API_KEY = '4ee7b1b87142cd25074b023d2a9a741a';
...
export default {
    getHomeList: async () => {
        return [
            {
                slug: 'originals',  //ID entendível em leitura humana
            ...
            },
            {
                slug: 'toprated',
                title: 'Em Alta',
                items: await basicFetch(`/discover/movie?${language}&api_key=${API_KEY}`),
            },
            {
                slug: 'action',
                title: 'Ação',
    ...
    getMovieInfo: async (movieId, type) => {
        ...
        if(movieId) {
            switch(type) {
                ...
                break;
                default:
                    alert('Erro na requisição ao procurar por mais informações!')
                    break;
            }
        }
